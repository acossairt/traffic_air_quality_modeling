...
plot1 = plot(x, y, seriestype=:scatter, ylims=(0, maximum(y) * 1.1),
    xlabel="Speed (km/h)", ylabel="Emissions (kg/hr)", markersize=6)
plot1 = plot!(xq, p_best.(xq), label="poly degree $bestdeg", linewidth=3)
plot1 = title!("Emissions vs speed — polynomial fit (deg=$bestdeg)", tickfontsize=14, guidefontsize=16, legendfontsize=14, titlefontsize=16)
savefig(plot1, "emissions_fit.pdf")

display(plot1)

#plot original data in g/mi vs speed in mph
xq_mph = [5:5:100]
plot2 = scatter(xq_mph, ygpm, markersize=6,
    label="", title="Emissions vs speed — original data",
    xlabel="Speed (mph)", ylabel="Emissions (g/mi)", tickfontsize=14, guidefontsize=16, titlefontsize=16)

savefig(plot2, "emissions_empirical.pdf")
display(plot2)