...
#define general velocity as a function of corridor average density adapted from Drake equation.
#Note "b" is a parameter that forces v to be v/2 at half jam density
#Note car_density = number cars/num lanes*length
v_gen(car_density,vel_ff,b,sharpness)= vel_ff*exp(-(car_density/b)^sharpness)
#calulate b:
b(half_jam,sharpness) = (half_jam) .* (1 ./ log(2)) .^ (1/sharpness)
#calucate car_density
c_den(nc,ψ,Le) = nc ./ (ψ .* Le)
avg_speed(nc,ψ,Le,vel_ff,half_jam,sharpness) = v_gen(c_den(nc,ψ,Le),vel_ff,b(c_den(half_jam,ψ,Le),sharpness),sharpness)
