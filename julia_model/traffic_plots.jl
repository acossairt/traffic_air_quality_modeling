...
#=
function speed_density_curve(; v_f, a, C, kc_half_jam)
    #kc_half = (1 ./ kc_jam) ./ 2
    return (v_f ./ ((pi / 2 .- atan.(a .* (0 .- kc_half_jam))) ./ pi)) .* (pi / 2 .- atan.(a .* (C .- kc_half_jam))) ./ pi
end
export speed_density_curve


function old_speed_density_curve(C, kc_half_jam)
    kc_jam = 1 ./ (kc_half_jam .* 2) # Is that really what it was?
    return exp.(-kc_jam .* C)
end
=#
...
function plot_results(sol)
    ...
    # Extract some parameter values which will be displayed
    my_L = sol.prob.ps[:L]
    my_r = sol.prob.ps[:r]
    my_x_0 = sol.prob.ps[:x_0]
    my_shift = sol.prob.ps[:shift]
    example_kc_half_jam = sol.prob.ps[:kc_half_jam][1, 2, 1] #kc_half_jam[i, j, k]
    example_v_f = sol.prob.ps[:v_f][1, 2, 1]
    my_a = sol.prob.ps[:a]
    ...
    # Function (hyper-local) to calc avg speeds
    #speed_density_curve(v_f, a, C, kc_half) = (v_f ./ ((pi / 2 .- atan.(a .* (0 .- kc_half))) ./ pi)) .* (pi / 2 .- atan.(a .* (C .- kc_half))) ./ pi
        ...
        for i in 1:Np # from patch i
            if j != i # skip diagonals (non-existent "loop" corridors)
                for k in 1:Nc # via corridor k
                    ...
                    #my_C_speeds = speed_density_curve.(v_f=this_v_f, a=this_a, C=sol[kc[i, j, k]], kc_half_jam=this_kc_half_jam)
    ...
    #= OLD VERSION
    # Test on corridor 1, p1 → p2
    my_speeds_test = speed_density_curve.(v_f=example_v_f, a=my_a, C=my_C_range, kc_half_jam=example_kc_half_jam)
    my_speeds_direct = speed_density_curve.(v_f=example_v_f, a=my_a, C=sol[kc[1, 2, 1]], kc_half_jam=example_kc_half_jam)
    my_speeds_old_test = old_speed_density_curve.(my_C_range, example_kc_half_jam)
    my_speeds_old_direct = old_speed_density_curve.(sol[kc[1, 2, 1]], example_kc_half_jam)
    plt4 = plot(my_C_range, my_speeds_test, label="speed-density relation", title="speed-density curve: c1, p1 → p2", ylabel="avg speed", xlabel="density")
    plt4 = plot!(plt4, my_C_range, my_speeds_old_test, label="old speed-density relation")
    plt4 = vline!(plt4, [(example_kc_half_jam) ./ 2], label="my C_half")
    plt4 = plot!(plt4, sol[kc[1, 2, 1]], my_speeds_direct, label="my speeds (directly calculated)", linewidth=3)
